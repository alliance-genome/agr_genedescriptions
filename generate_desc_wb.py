...
from genedescriptions.descriptions_rules import generate_ortholog_sentence_wormbase_human, \
...
def load_data(species, organism, conf_parser):
    ...
    return df, sister_df, df_agr, orthologs_sp_fullname
...
def set_do_sentence_and_set_stats(do_sentence_generator, do_sent_common_props, gene_desc, joined_sent):
...
def set_sister_species_sentence(df, sister_sp_fullname, sister_df, go_sent_gen_common_props, go_sent_common_props,
                                species, organism, gene_desc, joined_sent):
...
def generate_wormbase_description(gene: Gene, conf_parser: GenedescConfigParser, species, organism, df: WBDataFetcher,
    ...
    set_initial_go_stats(go_annotations=go_annotations, go_sent_generator=go_sent_generator,
                         go_sent_generator_exp=go_sent_generator_exp, gene_desc=gene_desc, conf_parser=conf_parser)
    ...
    set_do_sentence_and_set_stats(do_sentence_generator=do_sentence_generator,
                                  do_sent_common_props=do_sent_common_props, gene_desc=gene_desc,
                                  joined_sent=joined_sent)
    if not gene_desc.go_description:
        set_information_poor_sentence(orthologs_sp_fullname=orthologs_sp_fullname,
                                      selected_orthologs=selected_orthologs, ensembl_hgnc_ids_map=ensembl_hgnc_ids_map,
                                      conf_parser=conf_parser, human_df_agr=human_df_agr,
                                      go_sent_gen_common_props=go_sent_gen_common_props,
                                      go_sent_common_props=go_sent_common_props, gene_desc=gene_desc, df=df, gene=gene,
                                      joined_sent=joined_sent)
    if conf_parser.get_data_fetcher() == "wb_data_fetcher" and "main_sister_species" in species[organism] and \
        ...
        set_sister_species_sentence(df=df, sister_sp_fullname=sister_sp_fullname, sister_df=sister_df,
                                    ...
                                    species=species, organism=organism, gene_desc=gene_desc, joined_sent=joined_sent)
...
def main():
    ...
    for organism in organisms_list:
        ...
        df, sister_df, df_agr, orthologs_sp_fullname = load_data(species=species, organism=organism,
