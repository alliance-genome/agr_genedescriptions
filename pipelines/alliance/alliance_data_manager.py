...
class AllianceDataManager(DataManager):
    ...
    def load_annotations_from_api(self, associations_type: DataType, taxon_id: str, provider: str):
        if associations_type == DataType.GO:
            pass
        elif associations_type == DataType.EXPR:
            associations = []
            for gea in get_expression_annotations_from_api(data_provider=provider):
                associations.append(DataManager.create_annotation_record(
                    source_line="",
                    gene_id=gea["gene_id"],
                    gene_symbol=gea["gene_symbol"],
